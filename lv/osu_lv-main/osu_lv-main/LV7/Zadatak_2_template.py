...
#Ispisuje unique vrijednosti slike
colors = np.unique(img_array_aprox, axis=0)
print(len(colors))

# Zadatak 2

#Pravimo model sa n grupa (tj boja)
km = KMeans(n_clusters=10, init='random', n_init=5)
km.fit(img_array_aprox)
#Dobijamo za piksele kojoj grupi pripadaju (vrijednosti od 0 do 9 jer smo stavili gore 10 grupa)
labels = km.predict(img_array_aprox)

# Zadatak 3

#Svaki pixel priduzi određenoj grupi boje ili tako nesto
